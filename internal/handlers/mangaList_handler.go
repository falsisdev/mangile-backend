...
func GetMangaListHandler(c echo.Context) error {
	...
	if page != "" {
		...
		if err != nil || parsedPage < 1 {
			return c.JSON(http.StatusBadRequest, map[string]any{"code": 400, "message": "[HATA]: page parametresi geçerli bir pozitif sayı olmalıdır."})
		}
	}

	mangaList, err := services.GetMangaList(filterType, parsedLimit, parsedPage)
	if err != nil {
